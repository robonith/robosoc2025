...
interface Member extends Models.Document {
  ...
  linkedin?: string;
  instagram?: string; 
  insta?: string; // Added to match your JSX and fix the build error
  github?: string;
  techstack?: string[];
}
...
const Members: React.FC = () => {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      gsap.set(cardsRef.current.children, { opacity: 0, y: 50, scale: 0.9 });
            ...
            <div ref={cardsRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 justify-items-center">
              {allMembers.length === 0 ? (
                ...
                allMembers.map((member) => (
                  <div key={member.$id} className="group relative w-full max-w-[300px]">
                    <div className="bg-gray-900 rounded-2xl overflow-hidden border border-white/10 hover:border-white/30 transition-all duration-500">
                      ...
                      <div className="p-4 bg-white flex flex-col gap-2">
                        ...
                        <div className="flex gap-2 mt-2">
                          ...
                          {/* Supports both 'insta' and 'instagram' keys from database */}
